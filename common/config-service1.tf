resource "null_resource" "service1-config" {
  ...
  provisioner "file" {
    ...
    destination = "${var.service-instance-user-path}/service1-config.sh"
  }

  provisioner "remote-exec" {
    inline = [
      "apt install sudo -y",
      "chmod +x ${var.service-instance-user-path}/service1-config.sh",
      "sudo ${var.service-instance-user-path}/service1-config.sh"
    ]
  }
}
